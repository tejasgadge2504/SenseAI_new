...
class PatientHistoryScreen extends StatefulWidget {
  ...
  @override
  State<PatientHistoryScreen> createState() =>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            const RuraxTopBar(title: 'Patient History', showBack: true),
            Expanded(
              child: Column(
                children: [
                  // Search bar
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.people_outline,
                color: AppColors.darkGreen, size: 34),
          ),
          ...
          const Text('Register patients from the home screen.',
...
// ─── Patient detail: diagnosis history ──────────────────────────────────────
...
class _PatientDetailScreenState extends State<PatientDetailScreen> {
  ...
  Future<void> _load() async {
    final records =
    await StorageService.getDiagnosesForPatient(widget.patient.id);
    if (mounted) setState(() {
      _records = records;
      _loading = false;
    });
                      ...
                      Container(
                        ...
                        child: const Row(
                          children: [
                            ...
                            SizedBox(width: 10),
                            Text('No diagnoses recorded yet.',
                                style: TextStyle(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            // ── Top row: disease name + risk badge ───────────────────
            Row(
              children: [
                Expanded(
                  child: Text(r.diseaseType,
                      style: const TextStyle(
                          fontSize: 15,
                          fontWeight: FontWeight.w700,
                          color: AppColors.textDark)),
            ...
            // ── Bottom row: actions progress + view details ──────────
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                if (r.checklist.isNotEmpty)
                  Text(
                    '${r.checkedActions.length}/${r.checklist.length} actions done',
                    style: TextStyle(
                        fontSize: 12,
        ...
        builder: (_, ctrl) => SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              RiskBadge(level: r.riskLevel, score: r.riskScore),
              const SizedBox(height: 16),
              _detailSection('Explanation',
                  (r.apiResponse['explanation'] ?? '').toString()),
              _detailSection('Recommendation', r.recommendation),
              ...
              if (r.checklist.isNotEmpty) ...[
                ...
                if (r.checkedActions.isNotEmpty)
                  Padding(
                    ...
                    child: Text(
                      '${r.checkedActions.length} of ${r.checklist.length} actions completed',
                      style: const TextStyle(
                          fontSize: 12,
                          color: AppColors.darkGreen,
                          fontWeight: FontWeight.w600),
